...
export async function unfollowChannel(
  ...
  const [follower] = await db
    ...
    .where(
      and(
        eq(schema.channelFollowers.channelId, sourceChannelId),
        eq(schema.channelFollowers.webhookId, webhookId)
      )
    )
    .limit(1);
  ...
  await db
    ...
    .where(eq(schema.channelFollowers.id, follower.id));
}
...
export async function getChannelFollowers(channelId: string): Promise<ChannelFollower[]> {
  return await db
    .select()
    .from(schema.channelFollowers)
    .where(eq(schema.channelFollowers.channelId, channelId));
}
...
export async function getFollowerByWebhook(webhookId: string): Promise<ChannelFollower | null> {
  const [follower] = await db
    .select()
    .from(schema.channelFollowers)
    .where(eq(schema.channelFollowers.webhookId, webhookId))
    .limit(1);
